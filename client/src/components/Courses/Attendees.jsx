...
const Attendees = ({adminName, adminEmail, classCode}) => {
    ...
    useEffect(() => {
        setLoading(true);
        axios.post("http://localhost:5000/courses/getAttendees", {
            classCode: classCode
        },
        {
            headers: {
    ...
    return (
        <div className="content-box py-3 px-4 pb-4 mb-5">
            <div className="Attendees_Title mt-3">Course Creator</div>
            <hr className="m-0"></hr>
            ...
            <hr className="m-0"></hr>
            {
                loading ? <div className="d-flex justify-content-center mt-4 mb-2"><CircularProgress /></div> : attendees.length !== 0 ? 
                    ...
                    }) : <div className="mt-2">Currently there are no members enrolled in this course.</div>
            }
        </div>
    )
}
