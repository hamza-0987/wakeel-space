...
const Content = ({classCode, adminEmail}) => {
    const [discussionInput, setContentInput] = useState("");
 ...
 const history = useHistory();
  ...
  const [subject, setSubject] = useState();
    ...
    useEffect(() => {
        ...
        axios.post("http://localhost:5000/courses/getCourse", {
            classCode: classCode
    ...
    const getContent = async () => {
        setLoading(true);
        await axios.post("http://localhost:5000/courses/getContent",{
            classCode: classCode
        },{ headers: { Authorization: 'Bearer ' + storeData.token } }
        ).then ((res) => {
    ...
    const createContent = () => {
        ...
        if (fileInput) {
            ...
            const uploadTask = storage.ref(`Coursecontent/${fileName}`).put(fileInput);
            uploadTask.on('state_changed', console.log, console.error, () => {
                storage.ref('content').child(fileName).getDownloadURL()
                  .then(firebaseURL => {
                    return axios.post('http://localhost:5000/courses/createContent', {
                ...
                <>
                    ...
                    </div>
                    {
                        (discussionInput !== '' || fileInput) ? (
                            <div className="mt-2">
                                <SendRoundedIcon onClick={createContent} className="SendButton" style={{fontSize: "30px"}}/>
                            </div>
                        ) : null
                    }
                </>
